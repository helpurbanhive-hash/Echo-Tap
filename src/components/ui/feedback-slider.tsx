...
const HandDrawnSmileIcon = motion.create(
...
export interface FeedbackSliderProps
  extends Omit<HTMLMotionProps<"div">, 'onSelect'> {
...
const FeedbackSlider = React.forwardRef<HTMLDivElement, FeedbackSliderProps>(
  ({ className, onSelect, ...props }, ref) => {
    ...
    return (
      <motion.div
        ref={ref}
        className={`relative flex w-full items-center justify-center overflow-hidden rounded-3xl ${className}`}
        animate={{ backgroundColor: currentAnim.bgColor }}
        transition={transition}
        {...props}
      >
        <div className="flex h-full w-full max-w-[400px] flex-col items-center justify-center p-8">
          <motion.h3
            className="mb-10 w-full text-center text-xl font-semibold"
